...
def solution(h1, m1, s1, h2, m2, s2):
    # 시침, 분침, 초침들의 실제 값
    def real_time(hour: int, minute: int, second: float) -> Tuple[float, float, float]:
        return (hour + minute / 60 + second / 3600) % 12, minute + second / 60, second

    # 0시 0분 0초 기준으로 알람이 울린 횟수
    def get_alarm(hour: int, minute: int, second: float) -> int:
    ...
    def alarm_check(hour: int, minute: int, second: float) -> bool:
        real_hour, real_minute, real_second = real_time(hour, minute, second)
        if real_hour >= 12:
            real_hour -= 12
        # 알람이 울리면 True
